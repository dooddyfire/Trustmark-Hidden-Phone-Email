...
for i in range(start_page,end_page+1):
    ...
    url = "https://www.trustmarkthai.com/th/search?type_filter=0&total=20&page={}".format(i)
...
for detail_url in cust_detail:
    ...
    comments = soupx.find_all(string = lambda text: isinstance(text,Comment))
    lisa =[]
    a = 0
    for c in comments: 
        print("round = ",a) 
    ...
    print("test : ",lisa[1].extract())
  
    phone_email_lis = []
    for i in BeautifulSoup(lisa[1].extract(),'html.parser').find_all('tr'): 
        print(i.text.strip())
        phone_email_lis.append(i.text.strip())
        print("--------------------------")
  

    #phone = driver.find_element(By.XPATH,"/html/body/div[1]/table/tbody/tr[16]/td[2]").text 
    phone_lis.append(phone_email_lis[0].replace("โทรศัพท์ (Telephone) :","").strip())
    print(phone_email_lis[0].replace("โทรศัพท์ (Telephone) :","").strip())

    
    #email = driver.find_element(By.XPATH,"/html/body/div[1]/table/tbody/tr[18]/td[2]").text 
    

    mail = re.findall(r'[\w.+-]+@[\w-]+\.[\w.-]+', phone_email_lis[2])[0]
    print(mail)
    email_lis.append(mail)
